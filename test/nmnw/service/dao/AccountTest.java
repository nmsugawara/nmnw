...
public class AccountTest {
	...
	@Test
	public void setTokenExpireTimeTest() throws Exception{
		// 初期化
		Date expected = DateConversionUtility.stringToDate("2014-07-01");
		Account account = new Account();
		// 実行
		Field field = account.getClass().getDeclaredField("_tokenExpireTime");
		field.setAccessible(true);
		account.setTokenExpireTime(expected);
		// 検証
		assertEquals("setTokenExpireTime:正常系エラー", field.get(account), expected);
		// 後処理
	}

	@Test
	public void getTokenExpireTimeTest() throws Exception{
		// 初期化
		Date expected = DateConversionUtility.stringToDate("2014-07-01");
		Account account = new Account();
		// 実行
		...
		field.set(account, expected);
		// 検証
		assertEquals("getTokenExpireTime:正常系エラー", account.getTokenExpireTime(), expected);
		// 後処理
	}
